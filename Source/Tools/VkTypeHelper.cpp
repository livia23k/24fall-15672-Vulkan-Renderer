...
{
    std::unordered_map<std::string, VkPrimitiveTopology> primitiveTopologyMap = {
    ...
    std::unordered_map<std::string, VkIndexType> indexTypeMap = {
        {"UINT16", VK_INDEX_TYPE_UINT16},
        {"UINT32", VK_INDEX_TYPE_UINT32},
        {"NONE_KHR", VK_INDEX_TYPE_NONE_KHR},
        {"NONE_NV", VK_INDEX_TYPE_NONE_KHR},
        {"UINT8_KHR", VK_INDEX_TYPE_UINT8_KHR},
        {"UINT8_EXT", VK_INDEX_TYPE_UINT8_KHR}
    };

    // generated by ChatGPT 4o, sorry could not do it manually
...
std::optional<VkPrimitiveTopology> VkTypeHelper::findVkPrimitiveTopology(const std::string& topologyStr) {
    auto it = primitiveTopologyMap.find(topologyStr);
    if (it != primitiveTopologyMap.end()) {
        return it->second;
    } else {
        std::cerr << "Invalid VkPrimitiveTopology string: " << topologyStr << std::endl;
        return std::nullopt;
    }
}

std::optional<VkIndexType> VkTypeHelper::findVkIndexType(const std::string& indexTypeStr) {
    auto it = indexTypeMap.find(indexTypeStr);
    if (it != indexTypeMap.end()) {
        return it->second;
    } else {
        std::cerr << "Invalid VkIndexType string: " << indexTypeStr << std::endl;
        return std::nullopt;
    }
}

std::optional<VkFormat> VkTypeHelper::findVkFormat(const std::string& formatStr)
{
    auto it = formatMap.find(formatStr);
    if (it != formatMap.end()) {
        return it->second;
    } else {
        std::cerr << "Not supported VkFormat string: " << formatStr << std::endl;
        return std::nullopt;
    }
}