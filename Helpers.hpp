...
struct Helpers {
	...
	//allocate a block of requested size and alighment from a memory with the given type index
	Allocation allocate(VkDeviceSize size, VkDeviceSize alighment, uint32_t memory_type_index, MapFlag map = Unmapped);
